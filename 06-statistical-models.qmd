...
The data we have typically doesn't represent the full population about which we would like to say something. It might be difficult, for example, to measure the heights of more than 40 million women in Germany. Luckily, this is typically also not necessary. Under certain conditions, we can get a very good approximation of population characteristics by looking at only a (comparatively) small sample. The simplest way to get a sample which is *representative* of the underlying population is to pick units *randomly* (this might be easier said than done, as it might be difficult to circumvent selection biases).
