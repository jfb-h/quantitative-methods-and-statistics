...
1.  Create a variable called `my_age` with your age. Confirm that the variable exists and has the specified value in your global environment in RStudio.

```{r}

```

2.  Create another variable called `my_age_in_months` that multiplies your age by 12 (you can multiply two numbers with `*`).
...
5.  The `seq(from = ..., to = ..., by = ...)` function creates sequences of evenly spaced numbers. Use it to create a sequence of numbers from 1 to 100 in increments of 5. Write your code once with keyword arguments and one with positional arguments.
...
1. To ride the Oktoberfest roller coaster, you need to be at least 16 years old but also above 150 cm. Using logical expressions, write a piece of code which checks whether Lisa can ride the roller coaster:
...
2. Tim, Tom and Tina are 185cm, 173cm, and 175cm tall. Create a vector with their sizes. Then, write a piece of code to express their sizes in meters.

```{r}

```

3. The variable below represents a bike store's inventory. Turn it into a factor variable using the shorter labels "gravel", "city", and "racing" instead. Use the `summary()` function on your new variable. What does it tell you?
...
1.  Create a data frame with information about 3 of your favorite movies / series:
...
2.  Use `$` to access just the ratings column.

```{r}

```

3. Use `[]` to access the first row.
...
1. Install the `tidyverse` using RStudio's package installation interface.
