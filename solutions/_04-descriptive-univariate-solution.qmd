...
Here's a code snippet to create a combined dataset from the emissions and GDP data we saved previously, to use in the exercises:

```{r}
gdp <- arrow::read_parquet("data/gdp.parquet")
emi <- arrow::read_parquet("data/emissions-population.parquet")

df <- inner_join(gdp, emi, by = c("year", "code", "entity")) |>
    filter(str_length(code) == 3, year >= 2000, year <= 2023)
```

1.  Calculate the (population-weighted) mean and median GDP per capita. Which is larger and why?

2.  Install and load the `countrycode` package. Use it to create a new `region` column in your dataframe. You can use it like so:
...
df <- df |> mutate(
    region = countrycode::countrycode(
        code, origin = "iso3c", destination = "region")
)
```

3.  Compute weighted means and medians in each region, for the years 2000 and 2020. Plot the result.

```{r}
pd <- df |>
    filter(year == 2000 | year == 2023) |>
    drop_na(year, region) |> summarize(
        mean = weighted.mean(emissions, population),
        median = weighted.median(emissions, population),
        .by = c(year, region)
    ) |> arrange(region, year)

ggplot(pd, aes(mean, region, fill = factor(year))) +
    geom_col(position = "dodge") + labs(x="Mean per-capita GDP", y = "")
```

4.  Add new columns to your data frame with the growth rate of total regional GDP and total regional emissions. Plot the regional development of GDP and emissions for East Asia, Europe, and North America.

```{r}
pd <- df |> drop_na(
    region, year
    ) |> summarize(
        total_gdp = sum(gdp * population),
        total_emissions = sum(emissions * population),
        .by = c(year, region)
    ) |> mutate(
        growth_gdp = total_gdp / lag(total_gdp),
        growth_emissions = total_emissions / lag(total_emissions),
        .by = region
    ) |> arrange(region, year)

regions <- c("East Asia & Pacific", "North America", "Europe & Central Asia")

pd |> filter(region %in% regions) |> ggplot(
    aes(total_gdp, total_emissions, color = region)) +
    geom_line() +
    geom_text(aes(label = year)) +
    theme(legend.position = "bottom")
```

5.  Compute the average growth rates for emissions and gdp since the year 2000 for each region.

```{r}
pd |> drop_na() |> summarize(
    mean_growth_gdp = exp(mean(log(growth_gdp), na.rm = TRUE)),
    mean_growth_emissions = exp(mean(log(growth_emissions), na.rm = TRUE)),
    .by = region
)
```
