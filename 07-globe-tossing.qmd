...
Here, $Y_i$ is a random variable equal to 1 when the throw produces 'water', and 0 if it produces 'land'. The parameter $\theta$ is the underlying probability of observing 'water', which we want to learn about by observing how often our throws produce each of the two possible outcomes.
...
However, we usually don't know the model parameters beforehand and instead have to estimate them from data.
